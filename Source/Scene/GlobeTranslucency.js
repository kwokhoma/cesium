...
import GlobeTranslucencyMode from './GlobeTranslucencyMode.js';
    ...
    function updateCommands(globeTranslucency, context, width, height, oit, useOIT, scenePassState) {
        globeTranslucency._viewport.width = width;
        globeTranslucency._viewport.height = height;

        var useScissorTest = !BoundingRectangle.equals(globeTranslucency._viewport, scenePassState.viewport);
        var updateScissor = useScissorTest !== globeTranslucency._useScissorTest;
        globeTranslucency._useScissorTest = useScissorTest;

        if (!BoundingRectangle.equals(globeTranslucency._scissorRectangle, scenePassState.viewport)) {
            globeTranslucency._scissorRectangle = BoundingRectangle.clone(scenePassState.viewport, globeTranslucency._scissorRectangle);
            updateScissor = true;
        }
        ...
        if (!defined(globeTranslucency._passState)) {
            globeTranslucency._passState = new PassState(context);
        }
        ...
        globeTranslucency._passState.framebuffer = globeTranslucency._framebuffer;
        globeTranslucency._passState.scissorTest = scenePassState.scissorTest;
        globeTranslucency._passState.viewport = scenePassState.viewport;
    }
    ...
    function getTranslucentFrontFaceShaderProgram(context, shaderProgram) {
        return shaderProgram;
    }

    function getGlobeShaderProgram(context, shaderProgram) {
        return shaderProgram;
    }

    function getTranslucentShaderProgram(context, shaderProgram) {
        return shaderProgram;
    }
    ...
    derivedCommandPacks[GlobeTranslucencyMode.FRONT_FACES_ONLY] = {
        ...
        getShaderProgramFunctions : [getBackFaceShaderProgram, getFrontFaceShaderProgram, getTranslucentFrontFaceShaderProgram],
    ...
    derivedCommandPacks[GlobeTranslucencyMode.ENABLED] = {
        ...
        getShaderProgramFunctions : [getGlobeShaderProgram, getTranslucentShaderProgram],
    ...
    GlobeTranslucency.updateDerivedCommand = function(command, globeTranslucencyMode, context) {
        ...
        if (!defined(derivedCommands) || command.dirty) {
            ...
            if (!defined(shaders[0]) || (derivedCommands.shaderProgramId !== command.shaderProgram.id)) {
                ...
                for (i = 0; i < length; ++i) {
                    commands[i].shaderProgram = getShaderProgramFunctions[i](context, command.shaderProgram);
    ...
    GlobeTranslucency.prototype.updateAndClear = function(hdr, oit, useOIT, globeTranslucencyMode, viewport, context, scenePassState) {
        var width = viewport.width;
        var height = viewport.height;

        updateResources(this, context, width, height, hdr);
        updateCommands(this, context, width, height, oit, useOIT, scenePassState);
    ...
    GlobeTranslucency.prototype.executeGlobeCommands = function(commands, commandsLength, cameraUnderground, globeTranslucencyMode, executeCommandFunction, scene, context, scenePassState) {
        if (commandsLength === 0) {
            return;
        }
        ...
        executeCommands(commands, commandsLength, cullFace, false, executeCommandFunction, scene, context, scenePassState);
    };

    GlobeTranslucency.prototype.executeGlobeClassificationCommands = function(frustumCommands, cameraUnderground, globeTranslucencyMode, executeCommandFunction, scene, context, scenePassState) {
        ...
        if (globeTranslucencyMode === GlobeTranslucencyMode.FRONT_FACES_ONLY) {
            ...
            executeCommands(classificationCommands, classificationCommandsLength, undefined, false, executeCommandFunction, scene, context, scenePassState);
        ...
        executeCommands(globeCommands, globeCommandsLength, cullFace, true, executeCommandFunction, scene, context, this._passState);
        ...
        this._packedDepthCommand.execute(context, this._passState);
        context.uniformState.globeDepthTexture = this._packedDepthTexture;

        // Render classification
        executeCommands(classificationCommands, classificationCommandsLength, undefined, false, executeCommandFunction, scene, context, this._passState);
